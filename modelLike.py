...
def preprocess_user_car_profiles(user_car_profiles, numerical_features, categorical_features):
    if 'likes' in user_car_profiles and user_car_profiles['likes']:
        user_car_profiles_df = pd.json_normalize(user_car_profiles['likes'], sep='_')
    else:
...
def preprocess_car_profiles(car_profiles, numerical_features, categorical_features):
...
def calculate_cosine_similarity(user_features, car_features):
    return cosine_similarity(user_features, car_features)

def get_top_recommendations(cosine_sim, car_profiles):
